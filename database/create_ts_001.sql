...
BEGIN
    ...
    LOOP
      ...
      IF (activeTrade IS FALSE AND tradeDataRec.donchian_channel_30 = 'UPTREND') THEN
        tradeCommentary := '';
        tradeCommentary := nds.commentaryPrintLn(tradeCommentary, tradeDataRec.trade_date, 'trade identified');
      ...
      IF (activeTrade IS TRUE AND nds.isStopExitTriggered(tradeDirection, inExchangeName, inSymbol, tradeDataRec.trade_date, GREATEST(trailingStop,protectiveStop))) THEN
        ...
        UPDATE nds.trade_pair SET exit_date = tradeDataRec.trade_date, exit_price = GREATEST(trailingStop,protectiveStop), trade_commentary = tradeCommentary WHERE system_id = systemId AND exchange_name = inExchangeName AND symbol = inSymbol AND entry_date = entryRec.trade_date;
        activeTrade := FALSE;
      END IF;
